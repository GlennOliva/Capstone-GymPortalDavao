...
.service-card img {
    width: 200px; /* Adjust the size of the images/icons */
    height: auto;
    margin-bottom: 15px;
}
...
<div class="main-content">
    ...
    if (isset($_GET['id']) && isset($_GET['admin_id'])) {
        ...
        $sql = "SELECT * FROM tbl_service WHERE gym_id = ? AND admin_id = ?";
        $stmt = $conn->prepare($sql);
        if ($stmt) {
            ...
            if ($result->num_rows > 0) {
                ...
                while ($class = $result->fetch_assoc()) {
                    $service_image = $class['service_image'];
                    $service_name = $class['service_name'];
                    $service_description = $class['description'];
                    ?>
                   <div class="row">
        <!-- Cardio Machines -->
        <div class="col-md-3 text-center">
            <div class="service-card">
                <img src="../admin/services_image/<?php echo $service_image;?>" alt="Cardio Machines" class="img-fluid">
