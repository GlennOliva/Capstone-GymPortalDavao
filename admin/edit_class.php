...
@media screen and (max-width: 480px) {
	...
	<section id="sidebar">
        ...
        <li><a href="dashboard.php"><i class='bx bxs-dashboard icon'></i> Dashboard</a></li>
            ...
            <li><a href="gym_class.php"class="active" ><i class='bx bxs-chart icon'></i> Classes</a></li>
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <div class="image-upload-container">
            <!-- Current class image -->
            <img src="class_image/<?php echo $image; ?>" alt="Current Class Image" class="gym-image">
            <!-- Upload label -->
            <span class="upload-label">Upload Class Image</span>
            <!-- Upload input -->
        ...
        <!-- First row -->
        <div class="row">
            <input type="text" name="class_name" placeholder="Enter Class" value="<?php echo htmlspecialchars($class_name); ?>" required>
        </div>
        <!-- Second row -->
...
if (isset($_POST['update_class'])) {
    ...
    if (isset($_FILES['class_image']['name']) && $_FILES['class_image']['name'] != "") {
        ...
        $image_name = "Class-Image-" . rand(0000, 9999) . '.' . $ext; // Rename the image

        $source_path = $_FILES['class_image']['tmp_name'];
        $destination_path = "class_image/" . $image_name; // Folder to store the image
        ...
        if ($upload == false) {
            echo '<script>
                    swal({
                        ...
                        window.location = "edit_class.php?id=' . $id . '";
                    });
                </script>';
            die();
        }
    ...
    // Update query
    $sql = "UPDATE tbl_class SET gym_id='$gym_id', class_name='$class_name', description='$description', class_image='$image_name' WHERE id=$id";
    ...
    if ($res == true) {
        ...
        echo '<script>
                swal({
                    title: "Success",
                    text: "Class details successfully updated!",
                    icon: "success"
                }).then(function() {
                    window.location = "gym_class.php";
                });
            </script>';
    } else {
        ...
        echo '<script>
                swal({
                    title: "Error",
                    text: "Class details failed to update.",
                    icon: "error"
                }).then(function() {
                    window.location = "edit_class.php?id=' . $id . '";
                });
