...
@media screen and (max-width: 480px) {
	...
	<section id="sidebar">
		...
		<ul class="side-menu">
            ...
            <li><a href="gym_services.php" class="active"><i class='bx bxs-widget icon'></i> Services</a></li>
	...
	<section id="content">
		...
		<!-- MAIN -->
		<main>
            <h1 class="title">Edit Services</h1>
            <ul class="breadcrumbs">
                <li><a href="#">Admin</a></li>
                <li class="divider">/</li>
                <li><a href="#" class="active">Edit Services</a></li>
            </ul>
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <div class="image-upload-container">
            <!-- Current class image -->
            <img src="services_image/<?php echo $image; ?>" alt="Current Class Image" class="gym-image">
            <!-- Upload label -->
            <span class="upload-label">Upload Services Image</span>
            <!-- Upload input -->
        ...
        <!-- First row -->
        <div class="row">
            <input type="text" name="services_name" placeholder="Enter Class" value="<?php echo htmlspecialchars($services_name); ?>" required>
        </div>
        <!-- Second row -->
        ...
        <!-- Buttons -->
        <div class="buttons">
            <input type="submit" name="update_class" value="Edit" class="upload-btn">
        </div>
    </form>
</div>
...
if (isset($_POST['update_class'])) {
    ...
    if (isset($_FILES['class_image']['name']) && $_FILES['class_image']['name'] != "") {
        ...
        $image_name = "Service-Image-" . rand(0000, 9999) . '.' . $ext; // Rename the image

        $source_path = $_FILES['class_image']['tmp_name'];
        $destination_path = "services_image/" . $image_name; // Folder to store the image
        ...
        if ($upload == false) {
            echo '<script>
                    swal({
                        ...
                        window.location = "edit_services.php?id=' . $id . '";
                    });
                </script>';
            die();
        }
    ...
    // Update query
    $sql = "UPDATE tbl_service SET gym_id='$gym_id', service_name='$services_name', description='$description', service_image='$image_name' WHERE id=$id";
    ...
    if ($res == true) {
        ...
        echo '<script>
                swal({
                    title: "Success",
                    text: "Service details successfully updated!",
                    icon: "success"
                }).then(function() {
                    window.location = "gym_services.php";
                });
            </script>';
    } else {
        ...
        echo '<script>
                swal({
                    title: "Error",
                    text: "Service details failed to update.",
                    icon: "error"
                }).then(function() {
                    window.location = "edit_services.php?id=' . $id . '";
                });
