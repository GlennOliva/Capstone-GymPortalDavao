...
@media screen and (max-width: 480px) {
	...
	<section id="content">
		...
		<!-- MAIN -->
		<main>
            <h1 class="title">Insert About</h1>
            <ul class="breadcrumbs">
                <li><a href="#">Admin</a></li>
                <li class="divider">/</li>
                <li><a href="#" class="active">Insert About</a></li>
            </ul>
    ...
    <form method="POST" enctype="multipart/form-data">
        <div class="image-upload-container">
            ...
            <div class="image-upload">
                <input type="file" name="about_image" id="gym_image" required>
            </div>
        </div>
        
        <!-- Dropdown for existing gyms -->
        <div class="row">
        ...
        <!-- First row for purpose -->
        <div class="row">
            <input type="text" name="purpose" placeholder="Enter the purpose of the gym" required>
        </div>
        
        <!-- Second row for summary -->
        <textarea name="summary" placeholder="Enter the Goal, Vision, Target, and Brief Summary of your Gym" required></textarea>
        
        <!-- Buttons -->
        <div class="buttons">
            <input type="submit" class="upload-btn" name="add_about" value="Upload">
        </div>
    </form>
</div>
...
if(isset($_POST['add_about']))
{
    ...
    $gym_id = $_POST['gym_id'];
     ...
     if (isset($_FILES['about_image']['name']) && $_FILES['about_image']['name'] != "") {
        ...
        $image_name = "About-Image-" . rand(0000, 9999) . '.' . $ext; // Rename the image

        $source_path = $_FILES['about_image']['tmp_name'];
    ...
    // Insert query
    $sql = "INSERT INTO tbl_about (admin_id , gym_id , purpose , summary, about_image)
            VALUES ('$admin_id', '$gym_id', '$purpose', '$summary', '$image_name' )";
    ...
    if ($res == true) {
        ...
        echo '<script>
                swal({
                    title: "Success",
                    text: "About details successfully added!",
                    icon: "success"
                }).then(function() {
                    window.location = "gym_about.php";
                });
            </script>';
    } else {
        ...
        echo '<script>
                swal({
                    title: "Error",
                    text: "About details failed to add.",
                    icon: "error"
                }).then(function() {
                    window.location = "add_about.php";
                });
